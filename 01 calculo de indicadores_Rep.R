...
for (rep_actual in rep_vect){
  ...
  df_cons_abr24<- df_consultas %>% filter(hospital_42 == "SI")
  ...
  df_cons_abr24 <- filter(df_cons_abr24, hospital_42 == rep_actual) 
  ...
  plot_bars <- function(data, filename_suffix) {
    ...
    ggsave(plot = p, filename = here("02 graficas", paste("barra consulta_", rep_actual, "_", filename_suffix, ".png", sep = "")), width = 8.9527559055, height = 5.8740157, dpi = 300)
  }
  ...
  df_cons_abr24 <- bind_rows(df_cons_abr24, df_res2 %>% 
                               filter(tipo %in% unique(df_cons_abr24$tipo))) %>% 
    mutate(variacion = case_when(
      str_detect(nombre_corto, "Prom. Nacional") ~ "Promedio",
      incremento >= 15 ~ "Aumenta mucho",
      incremento > 5 & incremento < 15 ~ "Aumenta",
      incremento <= -15 ~ "Disminuye mucho",
      incremento < -5 & incremento > -15 ~ "Disminuye",
      TRUE ~ "Se mantiene"
    ),
  ...
  plot_consulta_diff_bars <- function(data, filename_suffix) {
    ggplot(data, aes(x = nombre_corto, 
                     y = incremento,
                     fill = variacion)) +
      ...
      geom_text(aes(x = nombre_corto, 
                ...
                size = 4, fontface = "bold", angle = 90,
                family = "Montserrat") +
      geom_text(data = data %>% filter(!is.na(nombre_ent)), aes(x = nombre_corto, 
                                                                y = posicion_text - ppgg*0.19,
                                                                hjust = posicion_text2,
                                                                color = variacion, label = paste0("(", comma(consultas_xconsult_23, accuracy = 0.1), "-", 
    ...
    ggsave(plot = last_plot(), filename = here("02 graficas", paste("barra consulta diff_", rep_actual, "_", filename_suffix, ".png", sep = "")), width = 8.9527559055, height = 5.8740157, dpi = 300)
  }
  ...
  df_cirugias <- filter(df_cirugias, !is.infinite(promedio_mes)) %>% 
    mutate(tipo=tipo_ordenado %>% 
             str_remove_all("^\\d*\\.\\s")) %>% filter(hospital_42 == "SI")
  
  df_cum <- read_excel(here("01 datos", "CUMM 2023_limpio.xlsx")) %>% 
    select(cve_pres, nombre, nombre_corto, ent_corta, consultorios, 
           tipo, tipo_ordenado, hospital_42) %>% 
    filter(hospital_42 == rep_actual & tipo_ordenado > 8) 
  ...
  df_existencias <- left_join(df_existencias, df_cum, by=c("clave"="cve_pres", "hospital_42")) %>% 
    mutate(prom_abasto = case_when(is.na(prom_abasto) ~ 0, 
                                   TRUE ~ prom_abasto)
    ) %>% 
    dplyr::filter(hospital_42==rep_actual) 
  ...
  df_existencias <- mutate(df_existencias, 
                           compara_prom=case_when(
                             ...
                             prom_abasto >= 95 & prom_abasto < 98 ~ "Mayor",
                             prom_abasto >= 92 & prom_abasto < 95 ~ "Igual",
                             prom_abasto < 92 & prom_abasto >= 88 ~ "Menor",
                             TRUE ~ "Mucho menor"
                           ))
  ...
  df_existencias <- mutate(df_existencias, 
                           compara_prom=case_when(
                             str_detect(nombre_corto, "Prom. Nacional") ~ "Promedio",
                             proporcion0 > prom_ent + sd_ent ~ "Mucho menor",
                             proporcion0 > prom_ent + 0.5 * sd_ent & proporcion0 <= prom_ent + sd_ent ~ "Menor",
                             proporcion0 < prom_ent - sd_ent ~ "Mucho mayor",
                             proporcion0 < prom_ent - 0.5 * sd_ent & proporcion0 >= prom_ent - sd_ent ~ "Mayor",
                             TRUE ~ "Igual"))
  ...
  df_existencias <- mutate(df_existencias, 
                           compara_prom=case_when(
                             ...
                             exist_prom > 0.15 & exist_prom <= 0.5 ~ "Mayor",
                             exist_prom >= -0.15 & exist_prom <= 0.15 ~ "Igual",
                             exist_prom >= -0.5 & exist_prom < -0.15 ~ "Menor",
  ...
  graf1 <- ggplot(df_existencias, aes(x = nombre_corto, 
    ...
    scale_fill_manual(values = c("Mucho mayor" = "#C14E15", "Mayor" = "#fee391", "Promedio"="#01665e",
                                 "Igual" = "#D9D9D9", "Menor" = "#c2a5cf", "Mucho menor" = "#9A2B95")) +
    scale_color_manual(values = c("Mucho mayor" = "black", "Mayor" = "black", "Promedio"="black",
                                  "Igual" = "black", "Menor" = "black", "Mucho menor" = "black")) +
  ...
  df_cirugias_nom <- fread(here("01 datos", 
    ...
    left_join(df_cum %>% select("cve_pre_uni_med"="cve_pres", tipo, hospital_42,
                                nombre_corto))
  ...
  graf1 <- ggplot(df_cirugias_nom, aes(x = nombre_corto, 
    ...
    geom_text(data = df_cirugias_nom, 
              aes(x = nombre_corto, 
                  ...
                  hjust = posicion_text2,
                  color = variacion,
                  label = paste0("(", comma(promedio_mes_ant, accuracy = 0.1), "-", 
                                 comma(promedio_mes, accuracy = 0.1), ")")), 
              size = 3, angle = 90,
              family = "Montserrat") +
    scale_fill_manual(values = c("Aumenta mucho" = "#4EA72E", "Aumenta" = "#a1d99b",  "Promedio"="#807dba",
  ...
  ppgg <- abs(max(df_nosocom$incremento, na.rm=T) - min(df_nosocom$incremento, na.rm=T))
  ...
  graf1 <- ggplot(df_nosocom, aes(x = nombre_corto, 
    ...
    geom_text(data = df_nosocom, 
              aes(x = nombre_corto, 
                  ...
                  hjust = posicion_text2,
                  color = variacion,
                  label = paste0("(", comma(promedio_mes_ant * 100, accuracy = 0.1), "-", 
  ...
  ppgg <- abs(max(df_estancia$promedio_mes, na.rm=T) - min(df_estancia$promedio_mes, na.rm=T))
  ...
  graf1 <- ggplot(df_quejas2, aes(x = nombre_corto, 
    ...
    scale_color_manual(values = c("Mucho mayor" = "black", "Mayor" = "black", "Promedio"="black",
                                  "Igual" = "black", "Menor" = "black", "Mucho menor" = "black")) +
    scale_y_continuous(name = "Inconformidades por cada 10 mil consultas")+
    theme_minimal() +
    labs(x="", y="")+
    theme(
      legend.title = element_blank(),
